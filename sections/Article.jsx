...
import { fadeIn, staggerContainer } from "../utils/motion";eret
          ...
          {articleList.map(({ title, img, uploadedAt, tags, description }, index) => (
            ...
            >
              <div className="h-[200px] overflow-hidden rounded-md relative">
                ...
                <div className="absolute top-[-100%] group-hover:top-2 left-2 transition-all duration-300">
                  {tags.map((tag, index) => (
                    <div
                      key={index}
                      className="bg-[#00B14F] text-white font-medium text-[0.8em] px-2 py-1 rounded-md"
                    >
                      {tag}
                    </div>
                  ))}
              ...
              <div className="flex flex-col gap-y-3">
                <h4 className="hover:text-[#00B14F] font-semibold text-[1em] cursor-pointer transition-all duration-300">
                  {title}
                </h4>
